...
AppLattice::AppLattice(SPPARKS *spk, int narg, char **arg) : App(spk,narg,arg)
{
  ...
  vav_prime_diff_tau_multiple = 1; //personalized
...
/*    std::cout << "\n\n" ;
    std::cout << "total_event_count_inside" ;
    std::cout << total_time_dt<< " " ;
    std::cout << "\n" ;
    std::cout << "\n\n" ;
    std::cout << "dt_inside" ;
    std::cout << dt<< " " ;
    std::cout << "\n" ;
    std::cout << "dtkmc_inside" ;
    std::cout << dt_kmc<< " " ;
    std::cout << "\n\n" ;*/
...
// Vav homogenized diffusion tau = l^2/D; = 8.5*8.5/10 = 7.225; here, l = half the length of sim box which is 17 micron.
    int time_count_vav = 0;
    if (time > (7.225* diff_tau_multiple)) {
      time_count_vav = time_count_vav + 1;

      if (time_count_vav >= 1) {
        diff_tau_multiple = diff_tau_multiple + 1;
        int total_cyt_vav = 0;
        int avg_cyt_vav = 0;
        for (int kk = 0; kk < 1156; kk++) {
          total_cyt_vav = population[9][kk] +total_cyt_vav;
        }
        avg_cyt_vav = total_cyt_vav/1156; 

        int total_spare_cyt_vav = 0;
        int total_loss_cyt_vav = 0;
        int remaining_cyt_vav = 0;

        for (int kk = 0; kk < 1156; kk++) {
          if (population[26][kk] != 0) {
            if (population[9][kk] > avg_cyt_vav) {
              int spare_cyt_vav = population[9][kk] - avg_cyt_vav;
              total_spare_cyt_vav = total_spare_cyt_vav + spare_cyt_vav;
              population[9][kk] = avg_cyt_vav;
            } else if (population[9][kk] < avg_cyt_vav) {
              int loss_cyt_vav = avg_cyt_vav - population[9][kk];
              total_loss_cyt_vav = total_loss_cyt_vav + loss_cyt_vav;
              population[9][kk] = avg_cyt_vav;
            }
          }
        }
        srand(0);
        remaining_cyt_vav = total_spare_cyt_vav - total_loss_cyt_vav;
        int kkk = 0;
        while (kkk < remaining_cyt_vav) {
           int r1 = 0;
           r1 = (rand() % 1156);
           if (population[26][r1] != 0 ) {
             population[9][r1] = population[9][r1] +1;
             kkk = kkk+1;
           }
        }
      }
...
// Shp homogenized diffusion tau = l^2/D; = 8.5*8.5/10 = 7.225; here, l = half the length of sim box which is 17 micron.
    int time_count_shp = 0;
    if (time > (7.225* shp_diff_tau_multiple)) {
      time_count_shp = time_count_shp + 1;

      if (time_count_shp >= 1) {
        shp_diff_tau_multiple = shp_diff_tau_multiple + 1;
        int total_cyt_shp = 0;
        int avg_cyt_shp = 0;
        for (int kk = 0; kk < 1156; kk++) {
          total_cyt_shp = population[21][kk] +total_cyt_shp;
        }
        avg_cyt_shp = total_cyt_shp/1156; 

        int total_spare_cyt_shp = 0;
        int total_loss_cyt_shp = 0;
        int remaining_cyt_shp = 0;

        for (int kk = 0; kk < 1156; kk++) {
          if (population[26][kk] != 0) {
            if (population[21][kk] > avg_cyt_shp) {
              int spare_cyt_shp = population[21][kk] - avg_cyt_shp;
              total_spare_cyt_shp = total_spare_cyt_shp + spare_cyt_shp;
              population[21][kk] = avg_cyt_shp;
            } else if (population[21][kk] < avg_cyt_shp) {
              int loss_cyt_shp = avg_cyt_shp - population[21][kk];
              total_loss_cyt_shp = total_loss_cyt_shp + loss_cyt_shp;
              population[21][kk] = avg_cyt_shp;
            }
          }
        }
        srand(0);
        remaining_cyt_shp = total_spare_cyt_shp - total_loss_cyt_shp;
        int kkk = 0;
        while (kkk < remaining_cyt_shp) {
           int r2 = 0;
           r2 = (rand() % 1156) ;
           if (population[26][r2] != 0 ) {
             population[21][r2] = population[21][r2] +1;
             kkk = kkk+1;
           }
        }
      }
    ...
    /*std::cout << "\n\n" ;
    std::cout << "population==" ;
    std::cout << population[2][80]<< " " ;*/
/*    std::cout << "\n" ;
    std::cout << "time_inside=" ;
    std::cout << time<< " " ;
    std::cout << "\n" ;
    std::cout << "dt_inside=" ;
    std::cout << dt<< " " ;
