...
void AppInbDiff::init_app()
{
  ...
  for (int nu = 0; nu <1156; nu++ ) {
    total_pvav = total_pvav + population[11][nu]+population[14][nu]+population[24][nu]+population[25][nu];
  }
}
...
void AppInbDiff::site_event(int i, class RandomPark *random)
{
  ...
  rxn_count[which]++;
    ...
    // update total number of pVav1 molecules based on the raction choosen
...
/
  if (which==25) {
    ...
    lck_cal_i_pop_present = population[5][j] +population[6][j]+population[7][j]+population[10][j]+population[11][j]+population[12][j]+population[13][j]+population[18][j]+population[19][j]+population[20][j]+population[22][j]+population[23][j]+population[24][j]+population[25][j]+population[8][j];
    cal_i_pop_present = population[5][j] +population[6][j]+population[7][j]+population[10][j]+population[11][j]+population[12][j]+population[13][j]+population[24][j];
