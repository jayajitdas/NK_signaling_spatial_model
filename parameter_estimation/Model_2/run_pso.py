...
def myfunc(x):
    ...
    try:
        ...
        if os.path.isfile(dir_name+"/sites.1.1"):
            if debug_flag:
            ...
            f = open(dir_name+"/sites.0.0_"+str(dir_name)+"adjusted15.15_60",'r')
            height_mean,height_stdev = calc_std_density(f)
            f.close()

            ff = open("sites.0.0_"+str(name)+"_thres_adjusted_S4_1_15.15",'r')
            height_mean_S4_2,height_stdev_S4_2 = calc_S4_std_density(ff)
            ff.close()
    ...
    try:
        ...
        shutil.copy2("sites_no_clus.30.30",dir_name)
        os.chdir(dir_name)
        setup_run(x)
        os.system("sbatch run.sh")
        sleep(2)
        setup_run_no_clus(x)
        os.system("sbatch run_no_clus.sh")
        sleep(2)
        os.chdir("..")
        if debug_flag:
            print('Started run')
